...
class PlayerService {
  ...
  resetToInitial() {
    ...
    this.level = 1;
    this.exp = 0;
    this.baseExpForNextLevel = 40;
    
    // Обновляем характеристики по уровню
  ...
  // Обновление характеристик в зависимости от уровня
  updateStatsByLevel() {
  ...
  // УСТРАНЯЕМ ЛОГИКУ РАСЧЕТОВ - только управление состоянием
  setHP(value) {
    this.hp = Math.max(0, Math.min(this.maxHp, value));
  }

  takeDamage(damage) {
    // Убираем логику расчета - просто применяем полученный урон
  ...
  // Для сброса только HP (между битвами)
  resetHP() {
    this.hp = this.maxHp;
  }
